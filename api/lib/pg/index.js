...
export const e = (qp, ...values) => {
    ...
    return new E(res + qp[qp.length - 1])
}
...
export const query = async (qp, ...values) => {
    ...
    let index = 1
    for (let i = 0; i < qp.length - 1; i++) {
        if (values[i] && values[i] instanceof LB) {
            query += qp[i]
        ...
        } else if (values[i] && values[i] instanceof E) {
...
export const lbjoin = (...args) => {
    ...
    for (let i = 0; i < args.length - 1; i++) {
        if (args[i] && args[i] instanceof LB) {
    ...
    if (
        args.length > 0 &&
        args[args.length - 1] &&
        args[args.length - 1] instanceof M
    ) {

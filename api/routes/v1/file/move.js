...
router.post('/:file_uuid/move', async ({ jwt: { payload: { email } }, params: { file_uuid }, body: { new_directory_uuid } }, res) => {
    ...
    if (!new_directory_uuid) {
        return JERROR_BAD_REQUEST(res, "no 'new_directory_uuid' specified in the payload")
    }
    ...
    if (!result) {
        return JERROR_INTERNAL_SERVER_ERROR(res, "check the 'file_uuid' parameter and the 'new_directory_uuid' payload.")
    }

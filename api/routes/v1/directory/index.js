...
import { E500, E400, JE500, JE12 } from '../lib/error'
...
router.post('/add', async ({ jwt: { payload }, body: { name, parent_directory_id } }, res, next) => {
    const parent_directory_uuid = xor_decode(parent_directory_id)

    let result = await addDirectory(payload.email, name, 0, parent_directory_uuid)
    if (!result) {
...
router.post('/rename', async ({ jwt: { payload }, body: { directory_id, new_name } }, res, next) => {
    const directory_uuid = xor_decode(directory_id)
    if (!directory_uuid || !new_name || new_name.length <= 0) {
        return res.json(JE12)
    }

    let result = await updateDirectory(payload.email, directory_uuid, { new_name })
    if (!result) {
        return res.json(JE500)
    }
    return res.json({
...
router.post('/move', async ({ jwt: { payload }, body: { directory_id, new_parent_directory_id } }, res, next) => {
    const directory_uuid = xor_decode(directory_id)
    ...
    let result = await updateDirectory(payload.email, directory_uuid, { new_parent_directory_uuid })
    if (!result) {
        return res.json(JE500)
    }
    return res.json({
...
router.post('/remove', async ({ jwt: { payload }, body: { directory_id } }, res, next) => {
    const directory_uuid = xor_decode(directory_id)
    if (!directory_id) {
        return res.json(JE1002)
    }

    let result = await deleteDirectory(payload.email, directory_id)
    if (!result) {
        return res.json(JE500)
    }
    return res.json({
